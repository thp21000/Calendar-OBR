...
export const conditionSummary = (project: CalendarProject, condition: WeatherCondition): string => {
  ...
  if (condition.type === "adventureContext") {
    ...
    const primaryIds = condition.primaryContextIds?.length ? condition.primaryContextIds : condition.contextIds;
    const labels = primaryIds.map((id) => state.availableContexts.find((context) => context.id === id)).filter((context): context is NonNullable<typeof context> => Boolean(context)).map((context) => `${context.icon} ${getAdventureContextLabel(context, locale)}`);
    return `${t(locale, "adventureContext.condition")} · ${t(locale, `adventureContext.localMode.${condition.contextRequirementMode ?? "primaryOnly"}`)} · ${labels.length > 0 ? labels.join(", ") : t(locale, "adventureContext.none")}`;
  }
...
export const WeatherEventForm = ({ project, event, mode, onSubmit, onCancel, inputStyle }: { project: CalendarProject; event: WeatherEvent; mode: "create" | "edit"; onSubmit: (event: WeatherEvent) => void; onCancel: () => void; inputStyle?: React.CSSProperties }) => {
  ...
  return <div>
    ...
    <WeatherEventFormSection title={t(project.locale, "eventDisplay.rules")}>
      <label style={field}><FieldLabel label={t(project.locale, "eventDisplay.familyId")} help={t(project.locale, "eventDisplay.familyHelp")} /><input value={draft.displayFamilyId ?? ""} onChange={(e) => updateDraft({ displayFamilyId: e.target.value.trim() || undefined })} style={mergedInputStyle} /></label>
      <label style={field}><FieldLabel label={t(project.locale, "eventDisplay.priority")} help={t(project.locale, "eventDisplay.priorityHelp")} /><input type="number" min={0} max={100} step={1} value={draft.displayPriority ?? 50} onChange={(e) => updateDraft({ displayPriority: Math.max(0, Math.min(100, Math.trunc(Number(e.target.value) || 0))) })} style={mergedInputStyle} /></label>
      <label style={field}><FieldLabel label={t(project.locale, "eventDisplay.severity")} help={t(project.locale, "eventDisplay.severityHelp")} /><select value={draft.displaySeverity ?? "standard"} onChange={(e) => updateDraft({ displaySeverity: e.target.value as WeatherEvent["displaySeverity"] })} style={mergedInputStyle}><option value="ambient">{t(project.locale, "eventDisplay.severity.ambient")}</option><option value="minor">{t(project.locale, "eventDisplay.severity.minor")}</option><option value="standard">{t(project.locale, "eventDisplay.severity.standard")}</option><option value="major">{t(project.locale, "eventDisplay.severity.major")}</option><option value="critical">{t(project.locale, "eventDisplay.severity.critical")}</option></select></label>
      <label style={field}><FieldLabel label={t(project.locale, "eventDisplay.weight")} help={t(project.locale, "eventDisplay.weightHelp")} /><input type="number" step={1} value={draft.displayWeight ?? 0} onChange={(e) => updateDraft({ displayWeight: Math.trunc(Number(e.target.value) || 0) })} style={mergedInputStyle} /></label>
      <label style={checkLabel}><input type="checkbox" checked={draft.ignoreDisplayLimit === true} onChange={(e) => updateDraft({ ignoreDisplayLimit: e.target.checked })} />{t(project.locale, "eventDisplay.ignoreLimit")}</label>
    </WeatherEventFormSection>
    ...
    <WeatherEventFormSection title={`${t(project.locale, "weatherEvents.conditions")} (${conditions.length})`}>
      ...
      {conditions.length === 0 ? <div style={hint}>{t(project.locale, "weatherEvents.noConditions")}</div> : conditions.map((condition, index) => <CollapsibleEditorBlock key={index} title={conditionSummary(project, condition)}>
        ...
        {condition.type === "adventureContext" ? <div style={condCard}>
          ...
          <label style={field}><div style={labelStyle}>{t(project.locale, "adventureContext.primaryContext")}</div><select value={condition.primaryContextIds?.[0] ?? condition.contextIds[0] ?? ""} onChange={(event) => updateDraftCondition(index, { ...condition, contextIds: event.target.value ? [event.target.value] : [], primaryContextIds: event.target.value ? [event.target.value] : [] })} style={mergedInputStyle}><option value="">{t(project.locale, "adventureContext.none")}</option>{normalizeAdventureContext(project.adventureContext).availableContexts.filter((context) => context.enabled).map((context) => <option key={context.id} value={context.id}>{context.icon} {getAdventureContextLabel(context, project.locale)}</option>)}</select></label>
