  ...
  return <div style={{ display: "grid", gap: 8 }}>
    ...
    {profiles.map((profile) => {
      ...
      <CollapsibleSection title={t(project.locale, "sceneWeather.skySection")}>
        <label style={{ display: "block" }}><FieldLabel label={t(project.locale, "weatherOverride.state")} help={t(project.locale, "sceneWeather.help.state")} /><select value={profile.override.state ?? ""} onChange={(event) => onProjectUpdate(patchOverride(project, profile, { state: (event.target.value || undefined) as WeatherState | undefined }))} style={inputStyle}><option value="" />{weatherStates.map((state) => <option key={state} value={state}>{getWeatherStateLabel(project, state)}</option>)}</select></label>
        <label style={{ display: "block" }}><FieldLabel label={t(project.locale, "weatherOverride.dominantState")} help={t(project.locale, "sceneWeather.help.dominantState")} /><select value={profile.override.dominantState ?? ""} onChange={(event) => onProjectUpdate(patchOverride(project, profile, { dominantState: (event.target.value || undefined) as WeatherState | undefined }))} style={inputStyle}><option value="" />{weatherStates.map((state) => <option key={state} value={state}>{getWeatherStateLabel(project, state)}</option>)}</select></label>
        <label style={{ display: "block" }}><FieldLabel label={t(project.locale, "weather.trend")} help={t(project.locale, "sceneWeather.help.trend")} /><select value={profile.override.trendKind ?? ""} onChange={(event) => onProjectUpdate(patchOverride(project, profile, { trendKind: (event.target.value || undefined) as WeatherTrendKind | undefined }))} style={inputStyle}><option value="" />{trendKinds.map((trend) => <option key={trend} value={trend}>{getWeatherTrendLabel(project, trend)}</option>)}</select></label>
      </CollapsibleSection>
