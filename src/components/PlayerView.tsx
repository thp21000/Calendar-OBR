...
export const PlayerView = ({ project, snapshot }: { project: CalendarProject; snapshot?: PublicCalendarTodaySnapshot | null }) => {
  if (snapshot) return <PlayerSnapshotView project={project} snapshot={snapshot} />;
  return <LocalPlayerPreview project={project} />;
};

const PlayerSnapshotView = ({ project, snapshot }: { project: CalendarProject; snapshot: PublicCalendarTodaySnapshot }) => {
          ...
          {snapshot.moons.length === 0 ? (
            ...
            <div style={{ display: "grid", gap: 2, fontSize: 12 }}>
              {snapshot.moons.map((moon) => (
                <div key={moon.name}>{moon.icon} {moon.name} — {t(locale, `moon.phase.${moon.phaseId}`)}</div>
              ))}
            </div>
          )}
        </div>
