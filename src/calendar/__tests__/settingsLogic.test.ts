...
describe("settingsLogic", () => {
  it("clamps too-high day to month max", () => {
    const result = clampDateParts(
      { year: 1000, monthId: "m2", dayOfMonth: 99, hour: 9, minute: 0 },
      system
    );
  ...
  it("prevents month days less than one", () => {
    const normalized = ensureValidCalendarSystem({
      ...system,
      months: [{ id: "m1", name: "X", order: 1, days: 0 }]
    });
  ...
  it("removing weekday clamps firstWeekdayOffset", () => {
    ...
    const removed = removeWeekday(scoped, "d2");
  ...
  it("invalid selected monthGridStartsOnWeekdayId falls back", () => {
    const ui: UiSettings = {
      activeTab: "month",
      compactMode: true,
      monthGridStartsOnWeekdayId: "missing"
    };
